...
const HeroSection = () => {
  ...
  const backgroundImages = {
    "/": {
      desktop:
        "https://cdn.jsdelivr.net/gh/akashsehdev/ntmgs-assets@main/ntmgs_bg.png",
      mobile: null, // We'll use video for mobile
    },
    "/talent": {
      desktop:
        "https://cdn.jsdelivr.net/gh/akashsehdev/ntmgs-assets@main/ntmgs_bg.png",
  ...
  // Sample mobile video (replace with your own later)
  const mobileVideo =
    "https://raw.githubusercontent.com/akashsehdev/ntmgs-assets/main/Untitled%20video%20-%20Made%20with%20Clipchamp.mp4";
  ...
  // Background element (image or video)
  let bgElement;
  if (location.pathname === "/" && isMobile) {
    // Mobile video for homepage
    bgElement = (
      <video
        autoPlay
        loop
    ...
    // Image for desktop or other pages
    const bgImage =
      location.pathname === "/" || location.pathname === "/talent"
        ? isMobile
          ? backgroundImages[location.pathname]?.mobile
          : backgroundImages[location.pathname]?.desktop
        : backgroundImages[location.pathname] || backgroundImages["/"].desktop;
  ...
  return (
    <div className="relative h-[600px] md:h-screen w-full transition-all duration-500">
      {/* Background */}
      {bgElement}
      ...
      {/* Content */}
      {/* <div className="absolute top-1/4 md:top-1/2 md:left-1/2 transform md:-translate-x-1/2 md:-translate-y-1/2 text-white text-center z-0">
        <h1 className="text-2xl mt-20 md:text-4xl font-serif font-bold drop-shadow-lg px-2">
          {currentContent.title}
        </h1>
        <p className="lg:block mt-2 md:mt-4 font-medium text-sm sm:text-base md:text-lg drop-shadow-md px-3">
          "{currentContent.description}"
        </p>
        <br />
        <br />
        <p className="mt-3 text-[12px] sm:text-base md:text-xl drop-shadow-md px-3">
          {currentContent.a}
        </p>
